...
let _lastDragRectTime = 0; // suppress spurious click after drag-to-place rectangle
...
function onMapClick(e) {
  ...
  // Pending shape: place it where the user tapped
  if (_pendingShape) {
    commitPendingShape(e.latlng);
    return;
  }
...
function parseMaterialListText(text) {
  ...
  String(text).split(/\r?\n/).forEach(function(line) {
    ...
    if (/^-{2,}/.test(t)) return; // '--- Section 1: ... ---' export headers
    // Prologue/footer lines from the app's own exports — never material rows
    if (/^(material list|customer:|address:|date:|generated by|total)/i.test(t)) return;
    ...
    if (qty === null) {
      // Leading-qty form — the app's own clipboard export: '18 4x4x8 PT line posts'
      var lead = t.match(/^(\d+(?:\.\d+)?)\s+(.+?)\s*$/);
      if (lead && !/^[\d$.]+$/.test(lead[2].split(/\s+/).pop())) {
        qty = Number(lead[1]);
        name = lead[2].trim();
      } else {
...
function initMulchDragHandlers() {
  ...
  map.on('mousemove', function(e) {
    ...
    // Detect missed mouseup (mouse left map and button was released externally)
...
function addMulchPoint(latlng) {
  ...
  if (_mulchMarkerDragging) return;
  // Suppress spurious click after drag-to-place rectangle
  if (Date.now() - _lastDragRectTime < 300) return;
...
function placeShape(shapeName) {
  ...
  _pendingShape = shapeName;
  if (currentTool !== 'mulch') setTool('mulch');
  hideShapePicker();
  showToast('Tap the map to place ' + shapeName);
}

function commitPendingShape(latlng) {
  var shapeName = _pendingShape;
  _pendingShape = null;
  var sizeFt = shapeDefaultSizes[shapeName] || 12;
  var points = generateShapePoints(shapeName, latlng.lat, latlng.lng, sizeFt);
...
function rebindMulchMarkerDrags(areaIdx) {
  ...
  area.polygon.off('click'); // Remove stale click handlers before rebinding
  ...
  // Touch tap fallback for delete mode (some browsers don't synthesize click from touch)
  var polyEl = area.polygon.getElement ? area.polygon.getElement() : null;
  if (polyEl) {
    polyEl._deleteTapHandler && polyEl.removeEventListener('touchend', polyEl._deleteTapHandler);
    polyEl._deleteTapHandler = function(te) {
      if (!_deleteMode) return;
      te.stopPropagation();
      var touch = te.changedTouches[0];
      var rect = map.getContainer().getBoundingClientRect();
      var latlng = map.containerPointToLatLng(L.point(touch.clientX - rect.left, touch.clientY - rect.top));
      selectMulchArea(areaIndex, latlng);
    };
    polyEl.addEventListener('touchend', polyEl._deleteTapHandler, { passive: false });
  }
  ...
  overlay.innerHTML =
    '<div class="modal" style="max-width:440px">' +
      ...
      '<div style="display:flex;flex-wrap:wrap;gap:8px;justify-content:center;margin-bottom:16px">' +
        ...
        '<a href="#" onclick="copyToClipboard(\'' + url.replace(/'/g, "\\'") + '\');showToast(\'Link copied — paste it in Discord\');return false;" style="display:flex;flex-direction:column;align-items:center;gap:4px;text-decoration:none;color:var(--text);font-size:0.72rem;padding:8px 10px;border-radius:var(--radius);border:1px solid var(--border);min-width:56px">' +
        ...
        '<a href="https://www.facebook.com/sharer/sharer.php?u=' + encodeURIComponent(url) + '" target="_blank" rel="noopener" style="display:flex;flex-direction:column;align-items:center;gap:4px;text-decoration:none;color:var(--text);font-size:0.72rem;padding:8px 10px;border-radius:var(--radius);border:1px solid var(--border);min-width:56px">' +
...
async function shareEstimate() {
  if (typeof requireTier === 'function' && !requireTier('contractor', 'send customer approvals')) return;
...
async function generatePDF(mode) {
  if (typeof requireTier === 'function' && !requireTier('builder', 'export PDF estimates')) return;
