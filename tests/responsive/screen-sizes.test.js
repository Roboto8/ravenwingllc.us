...
// =============================================================================
// NEW FEATURES — signup hints, disclaimer, modal behavior, touch targets
// =============================================================================
describe('New features across devices', () => {

  // ---- Company name hint visible on signup ----
  describe('signup company name hint', () => {
    test.each(Object.entries(DEVICES))('%s - field hint visible', async (name, { width, height }) => {
      const page = await browser.newPage();
      await page.setViewport({ width, height });
      await page.goto(INDEX_URL, { waitUntil: 'domcontentloaded', timeout: 10000 });
      await delay(300);
      ...
      const hint = await page.evaluate(() => {
        // Show the auth modal and signup form
        ...
        const el = document.querySelector('.field-hint');
        if (!el) return null;
        const rect = el.getBoundingClientRect();
        const style = getComputedStyle(el);
        return {
          ...
          text: el.textContent,
          fontSize: parseFloat(style.fontSize),
          withinViewport: rect.right <= document.documentElement.clientWidth
        };
      });

      expect(hint).not.toBeNull();
      expect(hint.visible).toBe(true);
      expect(hint.text.length).toBeGreaterThan(5); // not showing raw i18n key
      expect(hint.withinViewport).toBe(true);
      await page.close();
    }, 15000);
  });

  // ---- Estimate disclaimer visible and fits ----
  describe('estimate disclaimer', () => {
    test.each(Object.entries(DEVICES))('%s - disclaimer fits viewport', async (name, { width, height }) => {
      const page = await browser.newPage();
      await page.setViewport({ width, height });
      await page.goto(INDEX_URL, { waitUntil: 'domcontentloaded', timeout: 10000 });
      await delay(300);
      ...
      const disclaimer = await page.evaluate(() => {
        const el = document.querySelector('.estimate-disclaimer');
        if (!el) return null;
        const rect = el.getBoundingClientRect();
        const parent = el.closest('.panel-scroll') || el.parentElement;
        const parentRect = parent ? parent.getBoundingClientRect() : null;
        return {
          exists: true,
          text: el.textContent,
          fontSize: parseFloat(getComputedStyle(el).fontSize),
          fitsParent: !parentRect || rect.right <= parentRect.right + 2
        };
      });

      expect(disclaimer).not.toBeNull();
      expect(disclaimer.text).toContain('satellite');
      expect(disclaimer.fitsParent).toBe(true);
      await page.close();
    }, 15000);
  });

  // ---- Auth modal doesn't close when clicking inside ----
  describe('auth modal stopPropagation', () => {
    test('modal inner div has onclick stopPropagation', async () => {
      const page = await browser.newPage();
      await page.goto(INDEX_URL, { waitUntil: 'domcontentloaded', timeout: 10000 });
      await delay(300);
      ...
      const hasStopProp = await page.evaluate(() => {
        ...
        return onclick && onclick.includes('stopPropagation');
      });

      expect(hasStopProp).toBe(true);
      await page.close();
    }, 15000);
  });

  // ---- Fence vertex touch targets on touch devices ----
  describe('fence vertex touch expansion', () => {
    test('fence-vertex has ::before touch target rule in CSS', async () => {
      const page = await browser.newPage();
      ...
      const hasPosition = await page.evaluate(() => {
        ...
        // Check if .fence-vertex has position: relative (needed for ::before)
        const sheet = document.styleSheets[document.styleSheets.length - 1];
        try {
          for (const rule of sheet.cssRules) {
            if (rule.selectorText === '.fence-vertex' && rule.style.position === 'relative') {
              return true;
            }
          }
        } catch (e) {}
      ...
      // At minimum, verify the CSS class exists with the right properties
      const vertexStyle = await page.evaluate(() => {
        const el = document.createElement('div');
        el.className = 'fence-vertex';
        document.body.appendChild(el);
        const style = getComputedStyle(el);
        const result = { position: style.position, touchAction: style.touchAction };
        document.body.removeChild(el);
        return result;
      });

      expect(vertexStyle.position).toBe('relative');
      expect(vertexStyle.touchAction).toBe('none');
      await page.close();
    }, 15000);
  });

  // ---- Password placeholder shows correct requirement ----
  describe('password field placeholder', () => {
    test('signup password placeholder mentions 10+ chars', async () => {
      const page = await browser.newPage();
      await page.goto(INDEX_URL, { waitUntil: 'domcontentloaded', timeout: 10000 });
      await delay(300);
  ...
  // ---- Angle labels z-index below vertex markers ----
  describe('z-index stacking order', () => {
    test('labels z-index < markers z-index', async () => {
      const page = await browser.newPage();
      await page.goto(INDEX_URL, { waitUntil: 'domcontentloaded', timeout: 10000 });
      await delay(300);
      ...
      const zIndexes = await page.evaluate(() => {
        const results = {};
        const classes = ['fence-vertex', 'midpoint-handle', 'gate-marker', 'segment-label', 'angle-label', 'mulch-area-label'];
        classes.forEach(cls => {
          const el = document.createElement('div');
          el.className = cls;
          document.body.appendChild(el);
          results[cls] = getComputedStyle(el).zIndex;
          document.body.removeChild(el);
        });
        return results;
      });

      // Markers should be above labels
      const vertexZ = parseInt(zIndexes['fence-vertex']) || 0;
      const segLabelZ = parseInt(zIndexes['segment-label']) || 0;
      const angleLabelZ = parseInt(zIndexes['angle-label']) || 0;

      expect(vertexZ).toBeGreaterThan(segLabelZ);
      expect(vertexZ).toBeGreaterThan(angleLabelZ);
      await page.close();
    }, 15000);
  });

  // ---- Approval page disclaimer present ----
  describe('approval page disclaimer', () => {
    test.each([
      ['Desktop', { width: 1440, height: 900 }],
      ['Mobile', { width: 375, height: 667 }],
      ['Fold', { width: 280, height: 653 }]
    ])('%s - disclaimer visible on approval page', async (name, { width, height }) => {
      const page = await browser.newPage();
      await page.setViewport({ width, height });
      await page.goto(APPROVE_URL, { waitUntil: 'domcontentloaded', timeout: 10000 });
      await delay(300);
      ...
      const disclaimer = await page.evaluate(() => {
        // Find the disclaimer text near the total
        const els = document.querySelectorAll('p');
        for (const el of els) {
          if (el.textContent.includes('satellite')) {
            const rect = el.getBoundingClientRect();
            return {
              found: true,
              text: el.textContent,
              overflowing: rect.right > document.documentElement.clientWidth
            };
          }
        }
        return { found: false };
      ...
      expect(disclaimer.found).toBe(true);
      expect(disclaimer.overflowing).toBe(false);
      await page.close();
    }, 15000);
  });
});
